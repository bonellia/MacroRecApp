...
public class DashedCircleSeekbar extends View {
    ...
    private Drawable mIndicatorIcon2;
     ...
     * The current touch angle of arc.
     */
    private OnDashedCircleSeekbarChangeListener mOnDashedCircleSeekbarChangeListener;
    ...
    private void init(Context context, AttributeSet attrs) {
        ...
        mIndicatorIcon = ContextCompat.getDrawable(context, R.drawable.thumb_circular_seekbar);

        if (attrs != null) {
            // Attribute initialization
            final TypedArray a = context.obtainStyledAttributes(attrs,
                    R.styleable.DashedCircleSeekbar, 0, 0);

            Drawable indicatorIcon = a.getDrawable(R.styleable.DashedCircleSeekbar_indicatorIcon);
            ...
            mPoints = a.getInteger(R.styleable.DashedCircleSeekbar_points, mPoints);
            mMin = a.getInteger(R.styleable.DashedCircleSeekbar_min, mMin);
            mMax = a.getInteger(R.styleable.DashedCircleSeekbar_max, mMax);
            mStep = a.getInteger(R.styleable.DashedCircleSeekbar_step, mStep);

            mProgressWidth = (int) a.getDimension(R.styleable.DashedCircleSeekbar_progressWidth, mProgressWidth);
            progressColor = a.getColor(R.styleable.DashedCircleSeekbar_progressColor, progressColor);

            mArcWidth = (int) a.getDimension(R.styleable.DashedCircleSeekbar_arcWidth, mArcWidth);
            arcColor = a.getColor(R.styleable.DashedCircleSeekbar_arcColor, arcColor);

            mTextSize = (int) a.getDimension(R.styleable.DashedCircleSeekbar_textSize, mTextSize);
            textColor = a.getColor(R.styleable.DashedCircleSeekbar_textColor, textColor);

            mClockwise = a.getBoolean(R.styleable.DashedCircleSeekbar_clockwise,
                    mClockwise);
            mEnabled = a.getBoolean(R.styleable.DashedCircleSeekbar_enabled, mEnabled);
            a.recycle();
        }
        ...
        if (mEnabled) {
            ...
            switch (event.getAction()) {
                case MotionEvent.ACTION_DOWN:
                    if (mOnDashedCircleSeekbarChangeListener != null)
                        mOnDashedCircleSeekbarChangeListener.onStartTrackingTouch(this);
					updateOnTouch(event);
                    break;
                ...
                case MotionEvent.ACTION_UP:
                    if (mOnDashedCircleSeekbarChangeListener != null)
                        mOnDashedCircleSeekbarChangeListener.onStopTrackingTouch(this);
                    setPressed(false);
...
//				System.out.println("Reach Max " + progress);
                if (mOnDashedCircleSeekbarChangeListener != null) {
                    mOnDashedCircleSeekbarChangeListener
...
//				System.out.printf("-- %d, %d, %f\n", progress, mPoints, mProgressSweep);
    ...
    public interface OnDashedCircleSeekbarChangeListener {

        /**
         * Notification that the point value has changed.
         *
         * @param dashedCircleSeekbar The DashedCircleSeekbar view whose value has changed
         * @param points     The current point value.
         * @param fromUser   True if the point change was triggered by the user.
         */
        void onPointsChanged(DashedCircleSeekbar dashedCircleSeekbar, int points, boolean fromUser);

        void onStartTrackingTouch(DashedCircleSeekbar dashedCircleSeekbar);

        void onStopTrackingTouch(DashedCircleSeekbar dashedCircleSeekbar);
    }
    ...
    public void setOnDashedCircleSeekbarChangeListener(OnDashedCircleSeekbarChangeListener onDashedCircleSeekbarChangeListener) {
        mOnDashedCircleSeekbarChangeListener = onDashedCircleSeekbarChangeListener;
    }
}
